import { BiUser, BiSolidBookAlt, BiMicrophone, BiEnvelope, BiPhone } from 'react-icons/bi';

const lideres = [
  {
    name: 'Pastor Nome',
    cargo: 'Pastor Principal',
    desc: 'Responsável pela pregação e liderança espiritual da comunidade. Servindo com dedicação e amor há mais de 15 anos.',
    accent: '#D8C6A5',
    bg: 'bg-[#D8C6A5]/[0.04]',
  },
  {
    ...
    desc: 'Apoio pastoral no cuidado dos membros, aconselhamento e ministério da Palavra nos grupos pequenos.',
    accent: '#75836A',
    bg: 'bg-[#75836A]/[0.04]',
  },
  {
    ...
    desc: 'Conduz a equipe de louvor e adoração, preparando o coração da congregação para a presença de Deus.',
    accent: '#D8C6A5',
    bg: 'bg-[#D8C6A5]/[0.04]',
  }
];
...
function LiderCard({ m }) {
  return (
    ...
    >
      <div
        className="w-[120px] h-[120px] rounded-full flex items-center justify-center mb-5 transition-transform duration-300 group-hover:scale-105"
        style={{ backgroundColor: `${m.accent}20` }}
      >
        <BiUser className="text-[3rem]" style={{ color: `${m.accent}60` }} />
      </div>
      <h2 className="font-serif text-[1.4rem] font-normal mb-1 text-primary max-md:text-[1.2rem] relative z-10">{m.name}</h2>
      <span
        className="inline-block text-xs font-medium uppercase tracking-[0.15em] mb-4 px-3 py-1 rounded-full"
      ...
      <div className="flex gap-3 mt-auto">
        <a href="#" className="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 hover:-translate-y-0.5" style={{ backgroundColor: `${m.accent}15` }} aria-label="Enviar e-mail">
          <BiEnvelope className="text-base" style={{ color: m.accent }} />
        </a>
        <a href="#" className="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 hover:-translate-y-0.5" style={{ backgroundColor: `${m.accent}15` }} aria-label="Ligar">
          <BiPhone className="text-base" style={{ color: m.accent }} />
        </a>
      </div>
    </div>
  );
