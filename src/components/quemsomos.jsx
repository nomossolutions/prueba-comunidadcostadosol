...
function Card({ m }) {
  ...
  return (
    <div
      className={` bg-white rounded-xl p-10 text-dark flex flex-col items-start text-left relative overflow-hidden transition-all duration-400 group hover:-translate-y-1 max-md:p-6 ${m.bg}`}
      style={{
    ...
    >
      ...
      <h2 className="text-left w-full font-serif text-[1.5rem] font-medium mb-3 text-primary max-md:text-[1.25rem] relative z-10">
        {m.title}
      </h2>
      {m.desc && (
        <p className="text-[1.05rem] text-dark/80 leading-[1.7] max-md:text-base relative z-10">
          {m.desc}
        </p>
      )}
      {m.values && (
        <ul className="list-none pl-0 mb-6 w-full relative z-10 space-y-3">
          {m.values.map((v) => (
            <li
              key={v}
              className="text-dark/80 flex items-center text-[1.05rem] max-md:text-base"
            >
              <span className="flex items-center justify-center w-5 h-5 rounded-full bg-accent/20 mr-3 shrink-0">
                <BiCheck
                  className="text-accent-dark text-sm"
      ...
      <section className="w-full px-[5%] bg-light max-md:px-4 pb-24 scroll-reveal">
        <div className="max-w-[900px] mx-auto animate-fade-in-up">
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="text-accent-dark text-sm uppercase tracking-[0.2em] font-medium">
              Quem Somos
            </span>
          </div>
          <p className="font-serif text-[1.5rem] leading-[1.9] text-dark/80 max-w-[750px] max-md:text-[1.15rem]">
            Somos uma comunidade cristã que acredita em relacionamentos
            verdadeiros, cuidado mútuo e crescimento espiritual. Caminhamos
            juntos, acolhendo pessoas em cada fase da vida, com amor, fé e
            propósito.
          </p>
        </div>
      </section>
